...
const browserScript="(function(){'use strict';var pending=false,fallback;function update(){pending=false;var root=document.getElementById('root');if(!root||!document.body)return;if(!fallback){var holder=document.createElement('div');holder.innerHTML=FOOTER_VALUE;fallback=holder.firstElementChild;fallback.id='zain-fallback-footer';root.parentNode.insertBefore(fallback,root.nextSibling);}var links=root.querySelectorAll('a[href*=\"yetech.co\"]');for(var i=0;i<links.length;i++){var node=links[i];while(node&&node!==root){if(node.textContent.indexOf('Proudly made in Yemen')!==-1&&node.textContent.length<1500){if(node.style.display!=='none')node.style.display='none';break;}node=node.parentElement;}}}function schedule(){if(!pending){pending=true;requestAnimationFrame(update);}}function start(){update();var root=document.getElementById('root');if(root)new MutationObserver(schedule).observe(root,{childList:true,subtree:true});window.addEventListener('popstate',schedule);}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start);else start();})();".replace("FOOTER_VALUE",JSON.stringify(footer));
