 ...
 *entries(){for(const key of this.keys())yield [key,this.get(key)];}
 ...
 topFilter({keys,types,section,missing}={}){
  ...
  const where=[],args=[];if(keys){where.push(keys.length?'('+keys.map(()=> 'instr(name,?)>0').join(' OR ')+')':'0');args.push(...keys);}if(types){where.push(types.length?'type IN ('+types.map(()=>'?').join(',')+')':'0');args.push(...types);}if(section){where.push('section=?');args.push(section);}if(missing)where.push('has_content=0');
  const matches=row=>(!row.inItem||row.inItem==='null')&&(!keys||keys.some(key=>normalize(row.name).includes(key)))&&(!types||types.includes(row.type))&&(!section||row.sectionId===section)&&(!missing||!require('./metadata.cjs').hasContent(row));
...
module.exports=function(base){
 ...
 function signature(){return JSON.stringify({format:5,node:process.versions.v8,files:inputs.map(name=>{try{const s=fs.statSync(path.join(base,name));return name.endsWith('-wal')&&s.size===0?[name,0]:[name,s.size,s.mtimeMs,s.ctimeMs]}catch(e){if(e.code==='ENOENT')return [name,name.endsWith('-wal')?0:null];throw e;}})});}
 ...
 async function build(value,expected){let db;try{
  ...
  for(const name of families){db.exec('CREATE TABLE '+name+' (key TEXT PRIMARY KEY,value BLOB NOT NULL,summary TEXT)');const insert=db.prepare('INSERT INTO '+name+' VALUES(?,?,?)');counts[name]=snapshot[name].size;let batch=0;db.exec('BEGIN');
   for(const [key,row]of snapshot[name]){let summary=null;if(name==='items'&&(!row.inItem||row.inItem==='null')){const {content,files,...light}=row;summary=JSON.stringify({...light,__zainHasContent:require('./metadata.cjs').hasContent(row)});}insert.run(String(key),v8.serialize(row),summary);if(++batch%2000===0){db.exec('COMMIT');await tick();if(signature()!==expected)throw Error('Changed catalog');db.exec('BEGIN');}}
   db.exec('COMMIT');snapshot[name].clear();await tick();
  }
  db.exec('CREATE TABLE top_items (key TEXT PRIMARY KEY,summary TEXT,name TEXT,type TEXT,created REAL,section TEXT,has_content INTEGER)');const list=db.prepare('INSERT INTO top_items VALUES(?,?,?,?,?,?,?)');let n=0;db.exec('BEGIN');for(const row of db.prepare('SELECT key,summary FROM items WHERE summary IS NOT NULL ORDER BY rowid').iterate()){const item=JSON.parse(row.summary),name=String(item.name||'').replace(/[أإآ]/g,'ا').replace(/ة/g,'ه').replace(/[ؤئ]/g,'ء').replace(/ى/g,'ي').toLowerCase();list.run(row.key,row.summary,name,item.type||'',Number(item.createdAt)||0,item.sectionId||'',item.__zainHasContent?1:0);if(++n%2000===0){db.exec('COMMIT');await tick();db.exec('BEGIN');}}db.exec('COMMIT; CREATE INDEX top_type_date ON top_items(type,created DESC); CREATE INDEX top_date ON top_items(created DESC); CREATE INDEX top_missing_date ON top_items(has_content,created DESC); CREATE INDEX top_section_date ON top_items(section,created DESC)');
