...
module.exports = function createArtwork({ getItem, timeoutMs = 5000, concurrency = 8, maxCacheBytes = 32 * 1024 * 1024, workerPath = path.join(__dirname, 'artwork-worker.cjs') } = {}) {
  ...
  function pump() {
    while (!closed && running.size < concurrency && queue.length) {
      ...
      let settled = false;
      const finish = value => {
        if (settled) return; settled = true; clearTimeout(timer); running.delete(stop); pending.delete(job.key); child.kill();
        if (!closed) remember(job.key, value); job.resolve(value); pump();
      };
      const stop = () => finish(null);
      const timer = setTimeout(stop, timeoutMs);
      running.add(stop);
      child.once('message', result => finish(result && Buffer.isBuffer(result.bytes) ? result : null));
      child.once('error', stop); child.once('exit', stop); child.send({ candidates: job.candidates });
    }
  }
  return {
    peek(item) {
    ...
    close() { closed = true; for (const stop of [...running]) stop(); for (const job of queue.splice(0)) job.resolve(null); pending.clear(); cache.clear(); cacheBytes = 0; }
  };
};
