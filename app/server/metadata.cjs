...
module.exports=function({items,getKey,saveContent,savePoster,saveActors,hasArtwork,getLookupName,request,translateDescription,canTranslateDescription,onEvent=()=>{},concurrency=3,requestIntervalMs=100,retryDelayMs=1000}){
 ...
 async function sync(item,job){
  ...
  await Promise.all([
   (async()=>{if(!savePoster)return;try{check(job);const local=hasArtwork?await hasArtwork(item):false;check(job);if(local)return;if(!en.poster_path){job.warnings++;onEvent('لا توجد صورة متاحة في TMDB: '+item.name,'warning');return;}const result=await savePoster(item.id,en.poster_path);check(job);job.posters++;if(result?.status==='failed'){job.warnings++;onEvent('الصورة محفوظة مؤقتاً؛ تعذر الحفظ داخل مجلد '+item.name,'warning');}else onEvent('تم حفظ صورة داخل مجلد: '+item.name,'success');}catch(error){if(error.code!=='CANCELLED'){job.warnings++;onEvent('تعذر حفظ صورة '+item.name+'؛ بقيت بيانات المحتوى محفوظة','warning');}}})(),
 ...
 function enqueue(ids,{onlyMissing=false,automatic=false}={}){
  if(automatic&&(closed||!getKey()))return;if(closed)throw fail('الخدمة متوقفة',503);if(!getKey())throw fail('أدخل مفتاح TMDB في صفحة التحكم بالعناصر',422);
  ...
  const job={id:crypto.randomUUID(),ids:unique,total:unique.length,processed:0,completed:0,failed:0,skipped:0,posters:0,warnings:0,completedIds:[],activeItems:[],concurrency:workers,onlyMissing,automatic,errors:[],status:'queued',createdAt:new Date().toISOString()};jobs.push(job);while(jobs.length>20&&!['running','queued'].includes(jobs[0].status))jobs.shift();queue.push(job);onEvent('أضيفت مهمة مزامنة بيانات '+job.total+' عنصر إلى الخلفية','success');setImmediate(pump);return {body:{msg:'ok',jobId:job.id,total:job.total}};
 }
