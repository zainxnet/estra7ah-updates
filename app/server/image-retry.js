(()=>{'use strict';
 const posterStyle=document.createElement('style');posterStyle.textContent=".image-loading{background:#202638!important;border-radius:5px;animation:none!important}.image-loading>*{display:none!important}.image-loading[data-zain-pending=\"yes\"]{display:block!important}.zain-poster-slot{display:block;background:#202638;border-radius:8px;overflow:hidden}img[data-zain-artwork=\"queued\"],img[data-zain-artwork=\"loading\"]{aspect-ratio:2/3;background:#202638!important;color:transparent!important;visibility:hidden!important}img[data-zain-artwork=\"ready\"]{visibility:visible!important}";document.head.appendChild(posterStyle);
 ...
 function mark(img,status){img.setAttribute('data-zain-artwork',status);if(!window.CSS||!CSS.supports||!CSS.supports('aspect-ratio','2 / 3')){if(status==='queued'||status==='loading'){const width=img.getBoundingClientRect().width||img.parentElement?.clientWidth||100;img.style.minHeight=Math.round(width*1.5)+'px';}else img.style.minHeight='';}const p=img.parentElement;if(p)for(const child of p.children)if(child.classList.contains('image-loading'))child.setAttribute('data-zain-pending',status==='queued'||status==='loading'?'yes':'no');}
