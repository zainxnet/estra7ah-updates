...
module.exports=function createUiCompat({interfaceDir}){
 ...
 return {capabilities,bridgeScript:bridgeScript+adminTransport+seasonClickScript+";function zainFolderLabels(){document.querySelectorAll('.explor-btn .btn, .btn.play-btn').forEach(function(button){var label=button.textContent.trim();if(label==='تصفح'||label.indexOf('فتح المجلد')===0){Array.from(button.childNodes).forEach(function(node){if(node.nodeType===3&&node.textContent.trim()&&node.textContent!==' فتح المجلد وتصفحه')node.textContent=' فتح المجلد وتصفحه';});}});}zainFolderLabels();new MutationObserver(zainFolderLabels).observe(document.documentElement,{childList:true,subtree:true});"+';document.head.insertAdjacentHTML("beforeend",'+JSON.stringify(folderStyle+'<style>'+ ".image-loading{background:#202638!important;border-radius:5px;animation:none!important}.image-loading>*{display:none!important}.image-loading[data-zain-pending=\"yes\"]{display:block!important}.zain-poster-slot{display:block;background:#202638;border-radius:8px;overflow:hidden}img[data-zain-artwork=\"queued\"],img[data-zain-artwork=\"loading\"]{aspect-ratio:2/3;background:#202638!important;color:transparent!important;visibility:hidden!important}img[data-zain-artwork=\"ready\"]{visibility:visible!important}" +'</style>')+');',transform:route=>transformed.get(route)||null};
};
