...
(() => {
  ...
  function visible(img) {
    ...
    return r.bottom >= -100 && r.top <= innerHeight + 200 && r.right >= -100 && r.left <= innerWidth + 200;
  }
  function ready(img, s) {
    s.ready = true;
    s.awaitingNative = false;
    img.setAttribute('data-zain-artwork', 'ready');
  }
  function fail(img, s) {
    s.verified = false;
    s.awaitingNative = false;
    if (s.tries >= MAX_TRIES) {
      ...
      s.rendered = missing;
      img.src = missing;
      img.setAttribute('data-zain-artwork', 'unavailable');
      return;
    }
  ...
  function refresh() {
    scheduled = false;
    var _iterator = _createForOfIteratorHelper(tracked),
      _step;
    try {
  ...
  function discover(node) {
    ...
    var _iterator2 = _createForOfIteratorHelper(node.querySelectorAll('img')),
      _step2;
    try {
    ...
    try {
      for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
        ...
        if (record.type === 'attributes') state(record.target);else {
          var _iterator4 = _createForOfIteratorHelper(record.addedNodes),
            _step4;
          try {
  ...
  var _iterator5 = _createForOfIteratorHelper(document.images),
    _step5;
  try {
  ...
  window.addEventListener('online', () => {
    var _iterator6 = _createForOfIteratorHelper(tracked),
      _step6;
    try {
